...
int main(int argc, char *argv[]){
  ...
  while(count < cross){*/
      if(flag == 0){
        ...
        // We need to allocate the numbers array and the buffer
        char *inname = "matrix.txt"; 
        FILE *infile; 
        int *numbers_array = (int*)malloc(sz_file* sizeof(int));
        //malloc(sz_file* sizeof(int))
        ...
        while (fgets(line_buffer, sizeof(line_buffer), infile)) { 
            ...
            numbers_array[line_number] = atoi(line_buffer);
        }         
        fclose(infile);
